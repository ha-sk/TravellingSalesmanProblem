...
public class TSPDynamicprogramming {
    ...
    static int fun(int i, int mask) {
        ...
        // iterate through all cities j that have not been visited and are not the
        // starting city
        for (int j = 0; j < n; j++)
            if ((mask & (1 << j)) != 0 && j != i && j != 0) {
                int temp = fun(j, mask & (~(1 << i))) + dist[j][i];// recursively compute the shortest tour starting
                                                                   // from j, with i removed from the mask
                if (temp < res) {
                    res = temp;
                    prevCity = j;
                }
        ...
        prev[i][mask] = prevCity; // store the previous city in the optimal path
    ...
    public static void main(String[] args) {
        ...
        System.out.print("The optimal path is: " + cities[0] + " -> ");
        int mask = (1 << n) - 1;
        int currCity = startCity;
        // Traverse the optimal path in reverse order and print the city names
        while (currCity != 0) {
            System.out.print(cities[currCity] + " -> ");
            int nextMask = mask & (~(1 << currCity));
            int nextCity = prev[currCity][mask];
            mask = nextMask;
            currCity = nextCity;
        }
